...
filtR <- function(count_file, rho_CO=0.7, clr_CO=5, plot=FALSE) {
  ...
  if(plot == TRUE) {
    for(r in 1:nrow(final5)){
      plot.default(m.propr@logratio[,as.numeric(final5[r,1])], m.propr@logratio[,as.numeric(final5[r,2])], xlab = paste('CLR of OTU', final5[r,1]), ylab = paste('CLR of OTU', final5[r,2]), main = paste('Rho:', final5[r,3], ',', 'CLR Difference:', final5[r,4], ',', 'Slope:', lm(m.propr@logratio[,as.numeric(final5[r,2])]~m.propr@logratio[,as.numeric(final5[r,1])])$coefficients[2]))
